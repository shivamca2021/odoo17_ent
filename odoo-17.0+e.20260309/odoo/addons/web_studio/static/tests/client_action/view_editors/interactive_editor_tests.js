...
QUnit.module("View Editors", () => {
    QUnit.module("Interactive Editor", (hooks) => {
        ...
        QUnit.test("option of type 'string' are correctly saved and displayed", async (assert) => {
            patchWithCleanup(charField, {
                supportedOptions: [
                    {
                        label: "Suboption A",
            ...
            await createViewEditor({
                ...
                mockRPC: function (route, args) {
                    if (route === "/web_studio/edit_view") {
                        assert.strictEqual(args.operations[0].type, "attributes");
            ...
            await click(target.querySelector('[name="display_name"]').parentElement);
            assert.strictEqual(target.querySelector("input[id=suboption_a]").value, "cool cool");
            await editInput(target, "input[id=suboption_a]", "overriden"); // value overriden in route to make the assert relevant
            await contains("input[id=suboption_a]", { value: "Noice" });
        });
    });
});
