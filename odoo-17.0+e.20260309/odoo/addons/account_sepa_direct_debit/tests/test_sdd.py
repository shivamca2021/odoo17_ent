...
class SDDTest(AccountTestInvoicingCommon):
    ...
    def test_sdd_payment_with_closed_mandate(self):
        company = self.env.company
        company.sdd_creditor_identifier = 'BE30ZZZ300D000000042'

        # Setup company journal with IBAN
        company_bank_journal = self.company_data['default_journal_bank']
        company_bank_journal.bank_account_id = self.env['res.partner.bank'].create({
            'acc_number': 'BE38735538154772',
            'partner_id': company.partner_id.id,
            'bank_id': self.env['res.bank'].create({'name': 'Test Bank', 'bic': 'CTBEBEBB'}).id,
        })

        # Partner and bank account
        partner = self.env['res.partner'].create({'name': 'Test Partner'})
        partner_bank = self.create_account('DE44500105175407324931', partner, company_bank_journal.bank_account_id.bank_id)

        # Create and validate mandate
        mandate = self.create_mandate(partner, partner_bank, False, company, company_bank_journal)
        ...
        # Attempt payment -> should raise error
        with self.assertRaises(UserError):
            self.pay_with_mandate(invoice, mandate)
