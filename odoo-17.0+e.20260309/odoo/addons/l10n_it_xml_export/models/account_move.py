    ...
    def action_post(self):
        ...
        if closing_move and "l10n_it_xml_export_monthly_tax_report_options" not in self.env.context:
            ...
            if last_posted_tax_closing:
                ...
                if closing_move.date.month - last_posted_tax_closing[0].date.month > maximum_gap:
                    raise UserError(_("You cannot post the tax closing of %(month)s without posting the previous tax closing first.", month=closing_move.date.strftime("%m/%Y")))
            else:
