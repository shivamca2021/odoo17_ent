...
class TestMrpAnalyticAccountHr(TestMrpAnalyticAccount):
    ...
    def test_mrp_analytic_account_with_workorder(self):
        """
        Test adding and modifying analytic account to a confirmed manufacturing order with work orders.
        """
        ...
        with self.with_user(self.env.ref('base.user_admin').login):
            ...
            other_analytic_account = self.env['account.analytic.account'].create({
                'name': 'test_other_analytic_account',
                'plan_id': other_analytic_plan.id,
            })
            mo.analytic_distribution = {f'{self.analytic_account.id},{other_analytic_account.id}': 100.0}
            self.assertEqual(mo.analytic_account_ids, self.analytic_account + other_analytic_account)

            mo.workorder_ids[0].button_done()
            mo.workorder_ids[1].button_done()
