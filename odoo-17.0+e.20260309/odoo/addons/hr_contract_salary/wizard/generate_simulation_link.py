...
class GenerateSimulationLink(models.TransientModel):
    ...
    _sql_constraints = [
        ('check_validity', 'CHECK(validity >= 0)', 'The validity should not be negative.'),
    ]
    ...
    def _compute_url(self):
        for wizard in self:
            if wizard.contract_id:
                url = self.env['hr.contract'].get_base_url() + '/salary_package/simulation/contract/%s?' % (wizard.contract_id.id)
                params = {}
