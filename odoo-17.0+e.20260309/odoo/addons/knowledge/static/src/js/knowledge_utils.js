...
export function isDirty(field) {
    const strippedPropValue = stripHistoryIds(String(field.props.record.data[field.props.name]));
    const strippedEditingValue = stripHistoryIds(field.getEditingValue());
    const domParser = new DOMParser();
    const codeViewEl = field._getCodeViewEl();
    ...
    if (!codeViewEl && field.wysiwyg) {
        ...
        const editableValue = stripHistoryIds(field.wysiwyg.getValue({ $layout: $(editable) }));
        div.remove();
    ...
    return !field.props.readonly && parsedPreviousValue.innerHTML !== parsedNewValue.innerHTML;
}
