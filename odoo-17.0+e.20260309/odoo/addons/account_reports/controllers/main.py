...
class AccountReportController(http.Controller):
    ...
    def get_report(self, options, file_generator, **kwargs):
        ...
        try:
            ...
            if file_type in ('zip', 'xaf') or isinstance(file_content, types.GeneratorType):
    ...
    def _get_response_headers(self, file_type, file_name, file_content):
        ...
        if file_type in ('xml', 'txt', 'csv', 'kvr', 'csv') and not isinstance(file_content, types.GeneratorType):
