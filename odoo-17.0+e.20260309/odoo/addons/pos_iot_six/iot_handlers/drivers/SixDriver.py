...
class SixDriver(CtypesTerminalDriver):
    ...
    def processTransaction(self, transaction):
        ...
        # Transaction
        try:
            _logger.info('Performing Six transaction %s', transaction)
            ...
            if result == 1:
                _logger.info("Transaction %s approved", transaction)
                self.send_status(
                    response='Approved',
            ...
            # Transaction failed
                ...
                # If an error was encountered
                    ...
                    _logger.info("Transaction failed due to an error %s, %s", error_message, transaction)
    ...
    def cancelTransaction(self, transaction):
        ...
        try:
            if not TIMAPI.six_cancel_transaction(self.dev):
