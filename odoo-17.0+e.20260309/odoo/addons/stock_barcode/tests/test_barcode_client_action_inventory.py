...
class TestInventoryAdjustmentBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_gs1_inventory_lot_serial(self):
        ...
        product_serial_bis = self.env['product.product'].create({
            'name': 'PRO_GTIN_14_serial_bis',
            'type': 'product',
            'categ_id': self.ref('product.product_category_all'),
            'barcode': '15222222222226',  # GTIN-14 format
        ...
        smls_serial = self.env['stock.move.line'].search([
            ('product_id', 'in', (product_serial.id, product_serial_bis.id)),
            ('state', '=', 'done'),
        ...
        self.assertRecordValues(
            smls_serial,
            [
                {'product_id': product_serial.id, 'qty_done': 1},
                {'product_id': product_serial.id, 'qty_done': 1},
                {'product_id': product_serial.id, 'qty_done': 1},
                {'product_id': product_serial.id, 'qty_done': 20},
                {'product_id': product_serial.id, 'qty_done': 1},
                {'product_id': product_serial_bis.id, 'qty_done': 1},
            ],
        )
        self.assertEqual(
