...
registry.category("web_tour.tours").add('test_gs1_inventory_lot_serial', {test: true, steps: () => [
    ...
    // Scan the same product, an error notification should appear.
    {
        trigger: '.o_barcode_client_action',
        run: 'scan 011522222222221921Serial1',
    },
    ...
    // Scan a product with a different code bar, but same serial.
    // The following scanned barcode should be decomposed like that:
    //      - (01)15222222222226    > tracked by SN product barcode (15222222222226)
    //      - (21)Serial1           > serial number (Serial1)
    {
        trigger: '.o_barcode_client_action',
        run: 'scan 011522222222222621Serial1',
    },
    {
        trigger: '.o_barcode_line:contains("PRO_GTIN_14_serial_bis")',
        run: function () {
...
registry.category("web_tour.tours").add('test_gs1_delivery_ambiguous_lot_number', {test:true, steps: () => [
    ...
    },
    {
        ...
        run: function () {
            ...
            helper.assertLineIsHighlighted(line, true);
            helper.assertLineQty(line, "1");
            helper.assertLinesTrackingNumbers([line], ["12345"]);
        }
    },
