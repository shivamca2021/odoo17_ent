                ...
                if activity.mass_mailing_id.mailing_type == 'sms':
                    self.assertSMSTraces(
                        [{
                            ...
                            'check_sms': info.get('check_sms', True),
                            'failure_type': info.get('trace_failure_type', info.get('failure_type', False)),
                            'number': info.get('trace_sms_number', record.phone_sanitized),  # TDE FIXME: make it generic
