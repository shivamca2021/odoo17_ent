...
class AccountBankStatementLine(models.Model):
    ...
    def _get_default_amls_matching_domain(self):
        ...
        account_references = self.env['ir.property'].sudo()._read_group([
            ('name', 'in', account_stock_properties_names),
            ('company_id', '=', self.env.company.id),
            ('value_reference', '!=', False),
        ], groupby=['value_reference'])
        if account_references:
            account_ids = set()
            for (value_reference,) in account_references:
                _, account_id = value_reference.split(',')
                account_ids.add(int(account_id))
            blacklisted_stock_account_ids.update(account_ids)
