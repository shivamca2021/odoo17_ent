    ...
    def _compute_available_reserved_lots(self):
        ...
        for line in lines_to_check:
            ...
            for lot in line.reserved_lot_ids:
                ...
                if in_stock:
                    leaving_move_lines = leaving_move_by_lot.get(lot_id, self.env['stock.move.line'])
                    leaving = any(
                        leaving_move_line.move_id.date_deadline
                        and leaving_move_line.move_id.date_deadline <= line.return_date
                        and not (
                        ...
                        for leaving_move_line in leaving_move_lines
                    )
                    in_stock = not leaving
                else:
                    incoming_move_lines = incoming_move_by_lot.get(lot_id, self.env['stock.move.line'])
                    incoming = any(
                        ml.move_id.date_deadline
                        and ml.move_id.date_deadline <= line.reservation_begin
