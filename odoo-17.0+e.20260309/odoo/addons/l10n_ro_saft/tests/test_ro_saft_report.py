...
class TestRoSaftReport(TestAccountReportsCommon):
    ...
    def test_saft_zero_balance_partner(self):
        ...
        report = self.env.ref('account_reports.general_ledger_report')
        options = self._generate_options(report, fields.Date.from_string('2023-10-01'), fields.Date.from_string('2023-10-31'))
        self.env.flush_all()
        stringified_xml = self.env[report.custom_handler_model_name].l10n_ro_export_saft_to_xml(options)['file_content']
        with tools.file_open('l10n_ro_saft/tests/expected_xmls/saft_report_zero_balance_partner.xml', 'rb') as expected_xml_file:
            self.assertXmlTreeEqual(
                self.get_xml_tree_from_string(stringified_xml),
                self.get_xml_tree_from_string(expected_xml_file.read()),
            )
    ...
    def test_saft_zero_line(self):
        ...
        self.env.flush_all()
        stringified_xml = self.env[report.custom_handler_model_name].l10n_ro_export_saft_to_xml(options)['file_content']
        with tools.file_open('l10n_ro_saft/tests/expected_xmls/saft_report_zero_line.xml', 'rb') as expected_xml_file:
            self.assertXmlTreeEqual(
                self.get_xml_tree_from_string(stringified_xml),
                self.get_xml_tree_from_string(expected_xml_file.read()),
            )
    ...
    def test_saft_reverse_charge(self):
        ...
        self.env.flush_all()
        stringified_xml = self.env[report.custom_handler_model_name].l10n_ro_export_saft_to_xml(options)['file_content']
        with tools.file_open('l10n_ro_saft/tests/expected_xmls/saft_report_reverse_charge.xml', 'rb') as expected_xml_file:
            self.assertXmlTreeEqual(
                self.get_xml_tree_from_string(stringified_xml),
                self.get_xml_tree_from_string(expected_xml_file.read()),
            )
