...
class TestHrPayrollAccount(TestHrPayrollAccountCommon):
    ...
    @freeze_time("2010-01-22")
    def test_journal_entry_accounting_date(self):
        self.hr_contract_john.schedule_pay = 'semi-monthly'
        self.hr_payslip_john.date_from = date(2010, 1, 1)
        self.hr_payslip_john.date_to = date(2010, 1, 15)
        self.hr_payslip_john.compute_sheet()
        self.hr_payslip_john.action_payslip_done()

        self.assertEqual(
            self.hr_payslip_john.move_id.date,
            fields.Date.today(),
            'Accounting date should be same as todays date!'
        )
